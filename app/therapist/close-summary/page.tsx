...
import { recordCycleClose } from '@/app/therapist/actions/moat'
...
function CloseSummaryInner() {
  ...
  // DMT shadow: server action writes cycle_error + transition_record
  // Fire-and-forget — errors logged server-side, never block UI
  useEffect(() => {
    if (!cycle?.retest_baseline?.blocks || !cycleId) return
    recordCycleClose(cycleId).catch(() => { /* shadow — silent */ })
